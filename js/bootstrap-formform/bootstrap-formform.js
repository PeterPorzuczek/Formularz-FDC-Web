...
var FormForm = (function($) {
	return function(dom, fields) {
		...
		self.templates= {
			group: _.template(
				'<div class="form-group" id="<%= data.field.formId %>" style="display: block;">\
			...
			input: _.template(
				...
				, {variable: 'data'}),
			head: _.template(
				'<h3 name="<%= data.name %>" class="<%= data.class %>" id="<%= data.id %>"><%- data.value %></h3>'
				, {variable: 'data'}),
			head2: _.template(
				'<h4 name="<%= data.name %>" class="<%= data.class %>" id="<%= data.id %>"><%- data.value %></h4>\
				 <h6 name="<%= data.name %>" class="<%= data.classAdd %>" ><%- data.valueAdd %></h6>'
				, {variable: 'data'}),
			textarea: _.template(
			...
			button: _.template(
				'<button type="<%= data.type %>" class="btn <%= data.class %>"  <% if (data.id) { %> id="<%= data.id %>"<% } %> <% if (data.onClick) { %> onclick="<%= data.onClick %>"<% } %> >\
		...
		self.typeConfig = {
			...
			head: {
				template: self.templates.head,
			},
			head2: {
				template: self.templates.head2,
			},
			textarea: {
